        ...
        private  List<DataOne> FillingGrid(int direction, int userID)
        {
            ...
            if (direction == 1)
            {
                var userIsParticipant =
                    (from a in dataContext.Participants where a.active == true && a.fk_user == userID select a.fk_process)
                        .Distinct().ToList();

                var procesesToUser = (from p in userIsParticipant
                    join proc in dataContext.Processes on p equals proc.processID
                    where proc.active == true && proc.status == 0
                ...
                foreach (var proc in procesesToUser)
                {
                    var userQueue = (from a in dataContext.Participants
                        where a.fk_process == proc.processID && a.fk_user == userID && a.active == true
                    ...
                    var versionMax =
                        (from a in dataContext.ProcessVersions where a.active == true && a.fk_process == proc.processID select a)
                    ...
                    if (userQueue == 0)
                    {
                        if (proc.type.Equals("serial"))
                        {
                            var step4Proc =
                                (from a in dataContext.Steps
                                    where a.active == true && a.fk_processVersion == versionMax
                        ...
                        else
                        {
                            var step4UserParticipant = (from a in dataContext.Steps
                                where
                                    a.active == true && a.fk_participent == userParticipant && a.fk_processVersion == versionMax
                                select a).FirstOrDefault();
                    ...
                    else
                    {
                        ...
                        var lastStep =
                            (from a in dataContext.Steps where a.active == true && a.fk_processVersion == versionMax select a)
                        ...
                        if (lastStep != null)
                        {
                            ...
                            var lastUserQueue = (from a in dataContext.Participants
                                where a.active == true && a.fk_process == proc.processID && a.participantID == lastStepUser
            ...
            if (direction == 0)
            {
                var allProcessInitByUser =
                    (from a in dataContext.Processes where a.active == true && a.fk_initiator == userID select a).OrderByDescending(i => i.processID).ToList();
            ...
            {
                #region BEFORE_PORT
                /*var archive = (from a in dataContext.Participants where a.active == true && a.fk_user == userID
                                          join b in dataContext.ProcessVersions on a.fk_process equals b.fk_process where b.active == true
                                          join c in dataContext.Steps on a.participantID equals c.fk_participent where c.fk_processVersion == b.processVersionID && c.active == true
                                          select new DataOne() {Id = b.fk_process,
                                                                Version = b.version,
                                                                Name = (from p in dataContext.Processes where p.active == true && p.processID == b.fk_process select p.name).FirstOrDefault(),
                                                                Initiator = (from p in dataContext.Processes where p.active == true && p.processID == b.fk_process
                                ...
                                */
                #endregion BEFORE_PORT

                #region PORT
                var preArchive = (from a in dataContext.Participants where a.active == true && a.fk_user == userID
                                          join b in dataContext.ProcessVersions on a.fk_process equals b.fk_process where b.active == true
                                          join c in dataContext.Steps on a.participantID equals c.fk_participent where c.fk_processVersion == b.processVersionID && c.active == true
                                          select new  {b.fk_process,
                                                                    b.version,
                                                                    c.stepResult,
                                                                     b.status,
                                                                    c.date}
                ...
                List<DataOne> archive = preArchive.Select(itm => new DataOne()
                {
                    Id = itm.fk_process,
                    Version = itm.version,
                    Name = (from p in dataContext.Processes where p.active == true && p.processID == itm.fk_process select p.name).FirstOrDefault(),
                    Initiator = (from p in dataContext.Processes where p.active == true && p.processID == itm.fk_process
                                 join u in dataContext.Users on p.fk_initiator equals u.userID select u.name).FirstOrDefault(),
                    StatusParticipant = (itm.stepResult == 1) ? "Согласован" : "Отклонен",
                    Status4All = itm.status,
                    ParticipantApproveDate = itm.date
                }).ToList();

                dataOneSource.AddRange(archive.OrderByDescending(d => d.ParticipantApproveDate));
                ...
                case "StartP": 
                    {
                        ...
                        Processes process =
                            (from a in dataContext.Processes where a.active == true && a.processID == idProcess select a)
                                .FirstOrDefault();
                ...
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    if (direction == 0)
                    {
                     ...
                     //       e.Row.Cells[1].Text = "("+ procCharacterName + ")<br />"+ e.Row.Cells[1].Text;
                        ...
                        var procParent =
                            (from p in dc.Processes where p.active == true && p.processID == procId select p).FirstOrDefault();
                    ...
                    if (direction == 1)
                    {
                        ...
                        var isNew =
                            (from a in dc.Participants
                                where a.active == true && a.fk_process == processId && a.fk_user == userId
                        ...
                        int user = proc.fk_initiator; // PORT

                        var toltipData = (from u in dc.Users
                                          where u.active == true && u.userID == user
                        ...
                        {
                            Text = dc.Users.Where(u => u.active == true && u.userID == user).Select(u => u.name).FirstOrDefault(),

...
namespace EDM.edm
{
    public class EmailFuncs
    {
        ...
        public void StartProcess(int procId)
        {
            int currentQueue = 0;
            int procMaxVersion =
                (from a in dc.ProcessVersions where a.fk_process == procId && a.active == true select a)
                    .OrderByDescending(v => v.version).Select(v => v.processVersionID).FirstOrDefault();
            List<Steps> steps = (from a in dc.Steps where a.active == true == true && a.fk_processVersion == procMaxVersion select a).ToList(); // для AutoAccept() ContinueAccept
            ...
            EmailTemplates etmpInit =
                (from i in dc.EmailTemplates where i.active == true && i.name == "youStartedProcess" select i).FirstOrDefault();
            ...
            {       
                    ...
                    where a.active == true && a.fk_process == procId
                    join b in dc.Steps on a.participantID equals b.fk_participent
                    where b.active == true && b.fk_processVersion == procMaxVersion
            ...
            var participantsZero =
                (from a in dc.Participants where a.active == true && a.fk_process == procId && a.queue == currentQueue && a.isNew == true select a.fk_user)
                    .ToList();
            ...
            if (participantsZero.Any())
            {
                ...
                {
                    Users user =
                        (from a in dc.Users where a.active == true && a.userID == usrId select a).FirstOrDefault();
                    EmailTemplates etmp =
                        (from i in dc.EmailTemplates where i.active == true && i.name == "yourStep" select i).FirstOrDefault();
        ...
        {          
            ...
            */
            ...
            EmailTemplates etmpInit =
                (from i in dc.EmailTemplates where i.active == true && i.name == "yourProcessApproved" select i).FirstOrDefault();
            SendEmail(initiator.email, etmpInit.emailTitle, EmailContentAutoReplace(etmpInit.emailContent, procId,initiator.userID), null);
        }
        ...
        { 
            ...
            EmailTemplates etmpInit =
                (from i in dc.EmailTemplates where i.active == true && i.name == "yourProcessCameBack" select i).FirstOrDefault();
            SendEmail(initiator.email, etmpInit.emailTitle, EmailContentAutoReplace(etmpInit.emailContent,procId, initiator.userID), null);
        }
        public void StepApprove(int procId, int userId)
        {
            ...
            var userEmail =
                (from a in dc.Users where a.active == true && a.userID == userId select a.email).FirstOrDefault();
            EmailTemplates etmp =
                (from i in dc.EmailTemplates where i.active == true && i.name == "yourStep" select i).FirstOrDefault();
        ...
        public void AutoAccept()
        {
            ...
            var participantProcessDebt =
                (from a in dc.Participants where a.active == true && a.dateEnd < DateTime.Now
            ...
            {
                int procMaxVersion =
                    (from a in dc.ProcessVersions where a.fk_process == userProc.fk_process && a.active == true select a)
                     .OrderByDescending(v => v.version).Select(v => v.processVersionID).FirstOrDefault();
                ...
                var stepExsist = (from a in dc.Steps
                                  where
                                      a.fk_participent == userProc.participantID && a.fk_processVersion == procMaxVersion && a.active == true
                                  select a).FirstOrDefault();

                if (stepExsist == null)
                {
                    string processName = "";
                    EmailTemplates etmp = (from i in dc.EmailTemplates where i.active == true && i.name == "yourStepAutoSubmitted" select i).FirstOrDefault();
                    ...
                    var userEmail =
                        (from a in dc.Users where a.active == true && a.userID == userProc.fk_user select a.email)
                            .FirstOrDefault();

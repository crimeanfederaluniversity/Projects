...
namespace EDM.edm
{
    public partial class DocumentView : System.Web.UI.Page
    {
        ...
        public Panel GetChooseFromInnerProcessDocumentsPanel(int procId,int userId)
        {
            ...
            cancelButton.OnClientClick = "document.getElementById('ctl00_MainContent_chooseInnerProcPanel').style.visibility = 'hidden'; return false; ";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                if (procHasChild)
                {
                    ...
                    OpenFixedPanelButton.OnClientClick = "document.getElementById('ctl00_MainContent_chooseInnerProcPanel').style.visibility = 'visible'; return false; ";
                }

                int userID;
                int.TryParse(Session["userID"].ToString(), out userID);
                int procMaxVersion =
                    (from a in dataContext.ProcessVersions where a.fk_process == proc && a.active == true select a)
                ...
                var userParticInProc =
                    (from a in dataContext.Participants
                     where a.active == true && a.fk_user == (int)userId && a.fk_process == proc
                     select a.participantID).ToList();

                if (!userParticInProc.Any())
                {
                    Page.ClientScript.RegisterStartupScript(this.Page.GetType(), "err_msg", "alert('Вы не являетесь участником этого процесса. Ваши действия занесены в лог'); showSimpleLoadingScreen(); window.location.href = '../Default.aspx'", true);
                    // кикнуть
                    log.AddSecurity("Попытка утвердить процесс обманным способом");
                    Response.Redirect("~/Default.aspx");
                }

                var stepsForuser = (from a in dataContext.Steps
                    where a.active == true && a.fk_processVersion == procMaxVersion && a.fk_participent == userParticInProc[0]
                    select a).FirstOrDefault();

                if (stepsForuser != null)
                {
                    Page.ClientScript.RegisterStartupScript(this.Page.GetType(), "err_msg", "alert('Вы уже приняли участие в этом процессе'); showSimpleLoadingScreen(); window.location.href = '../Default.aspx'", true);
                    log.AddSecurity("Попытка утвердить процесс обманным способом (пользователь уже принял участие в этом процессе)");
                    Response.Redirect("~/Default.aspx");
                }
                ...
                ProcessVersions procVer =
                    (from b in dataContext.ProcessVersions where b.active == true && b.processVersionID == procMaxVersion select b)
                        .FirstOrDefault();
                ...
                LabelComment.Text =
                    (from a in dataContext.ProcessVersions
                     where a.active == true && a.processVersionID == procMaxVersion
                ...
                Participants part =
                    (from a in dataContext.Participants
                        where a.active == true && a.fk_user == userID && a.fk_process == proc
                        select a).FirstOrDefault();
                ...
                if (
                    (from a in dataContext.Processes where a.active == true && a.processID == proc select a.status)
                ...
                if (
                    (from a in dataContext.Processes where a.active == true && a.processID == proc select a.type).FirstOrDefault
                        () == "review")
                {
                    RejectButton.Enabled = false;
                ...
                var steps = (from a in dataContext.Participants where a.active == true && a.fk_user == userID && a.fk_process == proc
                             join b in dataContext.Steps on a.participantID equals b.fk_participent where b.active == true
        ...
        protected void ApproveButton_Click(object sender, EventArgs e)
        {
            ...
            int procMaxVersion =
                ...
                else if (AddStepFileFileUpload.HasFile == true)
                {
                    try
                    {
        ...
        protected void RejectButton_Click(object sender, EventArgs e)
        {
            //if (CommentTextBox.Text.Any())
            {
                ...
                if (!userParticInProc.Any())
                {
                    Page.ClientScript.RegisterStartupScript(this.Page.GetType(), "err_msg", "alert('Вы не являетесь участником этого процесса. Ваши действия занесены в лог'); showSimpleLoadingScreen(); window.location.href = '../Default.aspx'", true);
                    // кикнуть
                    log.AddSecurity("Попытка отклонить процесс обманным способом");
                    Response.Redirect("~/Default.aspx");
                }
